...
if (
    ...
    || (array_key_exists('systemZones', $payload) && !is_array($payload['systemZones']))
    || (array_key_exists('valuePatterns', $payload) && !is_array($payload['valuePatterns']))
) {
...
try {
    ...
    $nextRules['systemZones'] = normalize_system_zones($payload['systemZones'] ?? []);
    if (array_key_exists('valuePatterns', $payload)) {
